...
public class GuildLock implements Lock {
    private Guild owner;

    public GuildLock(Guild owner) {
        this.owner = owner;
    }
    ...
    public boolean canAccess(ArterionPlayer p) {
        ...
        return owner.getMember(p.getBukkitPlayer().getUniqueId()) != null;
    }

    @Override
    public void sendDeny(ArterionPlayer p) {
        p.scheduleHotbarCard(new InstantHotbarMessageCard(2000, p, "lock.guild.deny", p.getLanguage().translateObject(owner)));
    }

    @Override
    public void sendAllow(ArterionPlayer p) {
        p.scheduleHotbarCard(new InstantHotbarMessageCard(2000, p, "lock.guild.allow", p.getLanguage().translateObject(owner)));
    }
