...
public class CapturePointRegion extends Region {
    private CapturePoint capturePoint;

    public CapturePointRegion(CapturePoint capturePoint, World world, int x1, int x2, int z1, int z2) {
        super("region.capturepoint." + capturePoint.getRawName(), world, x1, x2, z1, z2, true, false, false, false, false, true);
    ...
    public void onEnter(ArterionPlayer player) {
        capturePoint.enterRegion(player);
    }

    @Override
    public void onLeave(ArterionPlayer player) {
        capturePoint.leaveRegion(player);
    }
}
