...
public class PlayerJobLevelCalculator {
    public static int getLevelFromXP(int xp) {
        ArterionFormula f = ArterionPlugin.getInstance().getFormulaManager().JOB_LEVEL_CURVE;
        int i = 0;
        for (; xp > 0; i++) {
            xp -= f.evaluateInt(i);
        }
    ...
    public static int getXPNeededForLevel(int level) {
        return ArterionPlugin.getInstance().getFormulaManager().JOB_LEVEL_CURVE.evaluateInt(level);
    }

    public static int getMaxLevel() {
        return ArterionPlugin.getInstance().getFormulaManager().JOB_LEVEL_MAX.evaluateInt();
    }
}
