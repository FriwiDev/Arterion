...
@Controller
public class StatArtefactFightController {
    @Autowired
    private MessageSource messageSource;

    @GetMapping("/stats/artefact_fight/{uuid}")
    public String artefact_fight(Locale locale, Model model, @PathVariable UUID uuid, @RequestParam(defaultValue = "damage_dealt_players") String detailstat, @RequestParam(defaultValue = "false") boolean timedependent) {
        ...
        db.beginTransaction();
        DatabaseArtefactFight fight = db.find(DatabaseArtefactFight.class, uuid);
        if (fight == null) {
            db.commit();
        ...
        model.addAttribute("carrier", fight.getCapturer());
        if (fight.getTimeEnd() == -1) {
            model.addAttribute("duration", messageSource.getMessage("page.stats.artefact_fight.broad.noduration", new Object[0], locale));
        } else {
        ...
        StatContextType contextType = StatContextType.ARTE_FIGHT;
        try {
        ...
        charts[0] = new String[]{
                messageSource.getMessage("page.stats.artefact_fight.stat_details_all", new Object[]{statName}, locale),
        ...
        for (Map.Entry<DatabaseGuild, DatabasePlayer[]> entry : guilds.entrySet()) {
            charts[1 + i] = new String[]{
                    messageSource.getMessage("page.stats.artefact_fight.stat_details_one", new Object[]{entry.getKey().getName(), statName}, locale),
