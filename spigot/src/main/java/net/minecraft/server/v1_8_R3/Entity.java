...
import net.minecraft.server.v1_8_R3.CommandObjectiveExecutor.EnumCommandResult;
import net.minecraft.server.v1_8_R3.EnumDirection.EnumAxis;
import net.minecraft.server.v1_8_R3.EnumDirection.EnumAxisDirection;
import net.minecraft.server.v1_8_R3.ShapeDetector.ShapeDetectorCollection;
...
public abstract class Entity implements ICommandListener {
    //TODO
    public boolean arterionInvisible = false;

    public void setArterionInvisible(boolean b){
        this.arterionInvisible = b;
        EntityTracker tracker = ((WorldServer)this.world).tracker;
        tracker.updatePlayersWithForce();
    }

    public boolean getArterionInvisible() {
        return arterionInvisible;
    }
    ...
    public boolean isAddedToChunk() {
        ...
        return this.ad && this.getChunkX() == chunkX && this.getChunkY() == chunkY || this.getChunkZ() == chunkZ;
    }
    ...
    private boolean paperNetherCheck() {
        return this.world.paperSpigotConfig.netherVoidTopDamage && this.world.getWorld().getEnvironment() == Environment.NETHER && this.locY >= 128.0D;
    }
        ...
        if (!this.fireProof) {
            ...
            if (this instanceof EntityLiving) {
                if (this.fireTicks <= 0) {
                    ...
                    EntityCombustEvent combustEvent = new EntityCombustByBlockEvent((Block)damager, damagee, 15);
    ...
    public void loadChunks() {
        for(int cx = (int)this.locX >> 4; cx <= (int)(this.locX + this.motX) >> 4; ++cx) {
            for(int cz = (int)this.locZ >> 4; cz <= (int)(this.locZ + this.motZ) >> 4; ++cz) {
                ((ChunkProviderServer)this.world.chunkProvider).getChunkAt(cx, cz);
            }
        }
    ...
    protected void a(double d0, boolean flag, net.minecraft.server.v1_8_R3.Block block, BlockPosition blockposition) {
        if (flag) {
    ...
    protected void Z() {
        ...
        if (block.b() != -1) {
            this.world.addParticle(EnumParticle.BLOCK_CRACK, this.locX + ((double)this.random.nextFloat() - 0.5D) * (double)this.width, this.getBoundingBox().b + 0.1D, this.locZ + ((double)this.random.nextFloat() - 0.5D) * (double)this.width, -this.motX * 4.0D, 1.5D, -this.motZ * 4.0D, new int[]{net.minecraft.server.v1_8_R3.Block.getCombinedId(iblockdata)});
        }
        ...
        try {
            ...
            if (this instanceof EntityLiving) {
                ...
                if (entity instanceof EntityTameableAnimal && !isLevelAtLeast(nbttagcompound, 2) && !nbttagcompound.getBoolean("PersistenceRequired")) {
                    bworld = (EntityInsentient)entity;
    ...
    public void c(int i) {
        if (!this.world.isClientSide && !this.dead) {
            ...
            if (this.dimension < 10) {
                Iterator var4 = minecraftserver.worlds.iterator();

                while(var4.hasNext()) {
                    WorldServer world = (WorldServer)var4.next();
