...
import me.friwi.recordable.impl.RecordablePlayer;
...
public class EntityTrackerEntry {
            ...
            if (this.tracker.vehicle == null) {
                ...
                if (object != null) {
                    if (object instanceof PacketPlayOutEntityTeleport) {
                        ...
                        Iterator var24 = this.trackedPlayerMap.entrySet().iterator();

                        while (var24.hasNext()) {
                            Entry<EntityPlayer, Boolean> viewer = (Entry) var24.next();
                            if ((Boolean) viewer.getValue()) {
                                ...
                                ((EntityPlayer) viewer.getKey()).playerConnection.sendPacket((Packet) object);
                            }
                        }
                    }
                }
    ...
    public void updatePlayer(EntityPlayer entityplayer) {
        ...
        if (entityplayer != this.tracker) {
            //if(tracker instanceof EntityPlayer)System.out.println("updatePlayer");
    ...
    public boolean c(EntityPlayer entityplayer) {
        ...
        //PacketCreationListener.init();
        //TODO Custom track range for our custom player
        if (entityplayer instanceof RecordablePlayer) {
            //Do not track gamemode 3 players as replay recorder
            if(tracker.arterionInvisible)return false;
            if(tracker instanceof EntityPlayer){
                if(((EntityPlayer) tracker).playerInteractManager.getGameMode() == WorldSettings.EnumGamemode.SPECTATOR)return false;
            }
            return ((RecordablePlayer) entityplayer).getRecording().isInViewRange(this.tracker.getWorld(), (int) this.tracker.locX, (int) this.tracker.locZ);
        }
        //TODO Remove invisible entities
        if(tracker.arterionInvisible)return false;

        double d0 = entityplayer.locX - this.tracker.locX;
        double d1 = entityplayer.locZ - this.tracker.locZ;
        /*if(tracker instanceof EntityPlayer){
            System.out.println(tracker.getId()+" "+(d0 >= (double) (-this.b) )+" "+( d0 <= (double) this.b )+" "+( d1 >= (double) (-this.b) )+" "+( d1 <= (double) this.b )+" "+( this.tracker.a(entityplayer)));
        }*/
    ...
    private boolean e(EntityPlayer entityplayer) {
        //TODO we are interested in everything that is not in gamemode 3
        if (entityplayer instanceof RecordablePlayer){
            if(tracker.arterionInvisible)return false;
            if(tracker instanceof EntityPlayer){
                if(((EntityPlayer) tracker).playerInteractManager.getGameMode() == WorldSettings.EnumGamemode.SPECTATOR)return false;
